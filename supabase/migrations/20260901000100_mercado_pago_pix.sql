...
alter table public.orders add constraint orders_payment_provider_check
  check (payment_provider is null or payment_provider in ('ASAAS', 'MERCADO_PAGO'));

update public.orders
set payment_provider = 'ASAAS', provider_payment_id = asaas_payment_id
where asaas_payment_id is not null and provider_payment_id is null;
...
begin
  ...
  if target_order.provider_payment_id is distinct from external_payment_id then raise exception 'payment does not belong to order'; end if;
  insert into public.payments(order_id, provider, external_id, status, amount, paid_at)
    ...
    on conflict (external_id) do update set provider = excluded.provider, status = excluded.status,
      paid_at = coalesce(public.payments.paid_at, excluded.paid_at);
  update public.orders set status = 'PAID', provider_payment_id = external_payment_id, paid_at = coalesce(paid_at, now()) where id = target_order.id;
