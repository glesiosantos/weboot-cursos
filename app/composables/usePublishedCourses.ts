...
export const usePublishedCourses = () => {
  ...
  return useAsyncData('published-courses', async (): Promise<Course[]> => {
    ...
    return Promise.all((data ?? []).map(async (course) => {
      ...
      return { ...course, workload_hours: Number(course.workload_hours), price: Number(course.price), promotional_price: course.promotional_price === null ? null : Number(course.promotional_price), current_batch: currentBatch ? { ...currentBatch, price: Number(currentBatch.price) } : null, public_price: course.pricing_type === 'BATCHES' ? (currentBatch ? Number(currentBatch.price) : null) : Number(course.promotional_price ?? course.price), cover_url: course.cover_path ? client.storage.from('course-covers').getPublicUrl(course.cover_path).data.publicUrl : null, instructor_name: Array.isArray(course.instructors) ? course.instructors[0]?.name ?? null : course.instructors?.name ?? null, presential: Array.isArray(course.course_presential_details) ? course.course_presential_details[0] ?? null : course.course_presential_details }
    }))
  })
}
