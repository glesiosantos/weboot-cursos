...
export default defineEventHandler(async (event) => {
  ...
  const { data: course, error } = await client.from('courses').select('id,title,slug,short_description,description,course_type,workload_hours,price,promotional_price,pricing_type,show_future_batches,cover_path,program,requirements,target_audience,instructors(name,bio),course_presential_details(location_name,address,address_number,city,state,starts_at,ends_at,max_students)').eq('slug', parsed.data).eq('status', 'PUBLISHED').is('archived_at', null).single()
  ...
  const { data: upcomingBatches, error: upcomingBatchError } = course.pricing_type === 'BATCHES' && course.show_future_batches ? await client.rpc('get_upcoming_course_batches', { target_course_id: course.id }) : { data: [], error: null }
  if (upcomingBatchError) { throw createError({ statusCode: 500, statusMessage: 'Falha ao carregar os próximos lotes' }) }
