...
export default defineEventHandler(async (event) => {
  ...
  const { data: course, error } = await client.from('courses').select('id,title,slug,short_description,description,course_type,workload_hours,price,promotional_price,cover_path,program,requirements,target_audience,instructors(name,bio),course_presential_details(location_name,address,address_number,city,state,starts_at,ends_at,max_students)').eq('slug', parsed.data).eq('status', 'PUBLISHED').is('archived_at', null).single()
